...
<div>
  ...
  <br/>
    ...
    <h3>지원자 이력서 다중 다운로드 문제2 - CORS</h3>
    <strong>▶ 문제 상황</strong>
    <p>로컬 환경(localhost)에서는 정상적으로 S3 파일 업로드와 다운로드가 작동했지만, AWS EC2 서버로 배포 후에는 CORS(Cross-Origin Resource Sharing) 오류가 발생했다. 브라우저 콘솔에는 “Access to fetch at ‘https://s3.amazonaws.com/...’ from origin ‘http://3.322.xxx.xxx’ has been blocked by CORS policy” 와 같은 메시지가 출력되며, S3 요청이 차단되었다.</p>
    <div>
      <img width="100%" height="auto" alt="4 zip문제2" src="https://github.com/user-attachments/assets/1bef99f6-3d26-4b5d-b21d-56ae980c5363" />
    </div>
    <strong>▶ 해결방법</strong>
    <p>원인을 확인해보니, S3 버킷의 CORS 설정에서 허용된 Origin이 http://localhost:10000으로만 지정되어 있었다. 즉, 실제 배포 서버 주소가 포함되지 않아, 도메인이 다른 요청이 보안 정책에 의해 거부된 것이다. <br/>이를 해결하기 위해 AWS S3 콘솔의 Permissions > CORS configuration으로 이동해 Origin 항목에 EC2 서버의 주소(http://3.322.xxx.xxx)를 추가했다. <br/>허용 메서드는 GET, PUT, POST, DELETE로 설정하고, 헤더는 *로 열어두었다. 변경 사항을 저장한 뒤 브라우저 캐시를 초기화하고 다시 요청을 보내자, 더 이상 CORS 차단이 발생하지 않고 정상적으로 파일 업로드 및 다운로드가 수행되었다. <br/>이 과정을 통해, 로컬 환경에서 문제없이 작동하더라도 배포 환경의 도메인이 달라지면 CORS 정책에 따라 접근이 제한될 수 있음을 다시 한 번 인식하게 되었다. 이후에는 배포 시점마다 Origin 정보를 환경 변수로 관리해 도메인 변경에 유연하게 대응할 수 있도록 개선했다.</p>
    <div>
      <img width="100%" height="auto" alt="4 zip문제해결2" src="https://github.com/user-attachments/assets/69ce626d-9494-4268-92dc-d658a7d5a1be" />
    </div>
...
보안과 유지보수 측면에서 더 적합하다고 판단했다.</p>
    ...
    <h3>10. 느낀점</h3>
<p>AWS S3 업로드, Bootpay 결제 연동, Redis 캐싱, 스케줄러 자동 상태 변경 등 단순한 CRUD를 넘어 서비스 전반의 흐름을 직접 설계하고 구현했다. 각자 담당한 페이지가 달랐지만, 기능 간 연결 구조가 많았기 때문에 서로의 코드를 공유하고 동작 방식을 맞춰가며 개발을 진행했다. 특히 캐시 적용 후 데이터 수정 시 캐시 무효화(@CacheEvict)가 누락된 문제는 팀원들과 로그를 함께 분석하며 원인을 찾아내 해결했고, S3의 presigned URL 적용 과정에서도 파일 접근 보안과 성능 간 균형을 함께 고민했다.</p> <p>개발 과정에서는 Mapper XML 오류, Enum 매핑 문제, 중복 키 예외, 캐시 미반영, 파일 null 값 등 다양한 오류를 마주했지만, 혼자 해결하기보다 팀 내에서 원인과 해결 방법을 공유하며 비슷한 문제가 다시 발생하지 않도록 개선 방향을 함께 논의했다. 그 덕분에 단순한 버그 수정이 아닌, “시스템 전체 안정성을 함께 높여가는 과정”을 경험할 수 있었다.</p> <p>프로젝트를 진행하며 느낀 건, 유지보수가 쉬운 시스템은 결국 서로가 코드를 이해하고 공유하는 팀 문화에서 나온다는 점이었다. 한 기능을 완성하는 것보다, '서로의 로직이 자연스럽게 이어지는 구조'를 만드는 일이 훨씬 중요하다는 걸 깨달았다.</p> <p>이번 프로젝트를 통해 “백엔드”는 단순히 서버 로직을 작성하는 역할이 아니라, 팀의 방향성과 데이터 흐름을 함께 설계하는 중심 역할이라는 걸 느꼈다. 이제는 기능 구현을 넘어, '소통과 협력 속에서 안정적이고 확장 가능한 구조를 설계하는 개발자'로 성장하고 싶다.</p>
